...
export const ShutdownSystem = (props: any) => {
    ...
    const onFinish = async () => {
        const reboot = await ubusApi.config_system_shutdown()
        setShowModal(false)
    }
    ...
    return (
        <Card title={t("shutdown")} type="inner" headStyle={{ background: "linear-gradient(113deg, rgba(32,162,145,1) 0%, rgba(18,207,141,1) 30%)", color: 'white', }}>
            <Space
                direction="vertical"
                size="middle"
                style={{

...
export const GreEdit = (props: any) => {
    ...
    const refreshData = async () => {
        if (item) {
            const {status, name, peeraddr, ipaddr, network, netmask } = item
            form.setFieldsValue({
                status, name, peeraddr, ipaddr, network, netmask,
            })
        }
    }
    React.useEffect(() => {
    ...
    const onFinish = async (values: any) => {
        if (values.peeraddr == null && values.ipaddr == null && values.network == null && values.netmask == null) {
            const key = 'fail';
            ...
            await ubusApi.config_gre( "enable", values.name, values.peeraddr, values.ipaddr, values.network, values.netmask)
            const key = 'success';
            message.loading({ content: t('loading'), key });
            setTimeout(() => {
    ...
    return (
        <Form
            ...
            onFinish={onFinish}
        >
            <Form.Item label={t('Gre Name')} name="name"
                rules={[
            ...
            <Form.Item label={t('local_address')} name="peeraddr"
                rules={[
            ...
            <Form.Item label={t('remote_address')} name="ipaddr"
                rules={[
            ...
            <Form.Item label={t('network')} name="network"
                rules={[

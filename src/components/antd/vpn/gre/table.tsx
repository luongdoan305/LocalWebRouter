...
export const GreTable = (props: any) => {
  ...
  const confirmDeleteGroup = async (group: any) => {
    const {name, peeraddr, ipaddr, network, netmask } = group
    const deleterules = await ubusApi.config_gre("disable", name, peeraddr, ipaddr, network, netmask)
  ...
  const columns = [
    { title: t('Gre Name'), key: 'gre_name', dataIndex: 'name' },
    { title: t('local_address'), key: 'local_address', dataIndex: 'peeraddr' },
    { title: t('remote_address'), key: 'remote_address', dataIndex: 'ipaddr' },

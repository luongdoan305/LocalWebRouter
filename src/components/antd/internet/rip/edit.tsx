...
export const Ripedit = (props: any) => {
    ...
    const refreshData = async () => {
        if (item) {
            const { mode, network, subnet } = item
            form.setFieldsValue({
                mode, network, subnet
            })
        }
    ...
    const onFinish = async (values: any) => {
        ...
        console.log(values.mode, network_conf )
        const nat = await ubusApi.config_network_rip("enable", values.mode, network_conf, "false", "false", "false", "false", "false", "false")
        const key = 'updatable';
    ...
    return (
        <Form
            ...
            onFinish={onFinish}
        >
            <Form.Item label={t('mode')} name="mode" required={true}>
                <Select
                    options={[
                        {
                            value: "IPv4",

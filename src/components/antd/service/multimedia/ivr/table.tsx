...
export const IvrTable = (props: any) => {
  ...
  const confirmDeleteGroup = async (group: any) => {
    const { button_ivr, music, button, name, number } = group
    //const deleterules = await ubusApi.config_media_ivr( "disable", button_ivr, music, button, name, number)
    const deleterules = await ubusApi.config_media_ivr("disable", button_ivr, music, button, name, number)
  ...
  const columns = [
    { title: t('Number IVR'), key: 'number_ivr', dataIndex: 'button_ivr' },
    { title: t('Music'), key: 'mucsic', dataIndex: 'music' },
    {
      title: t('action'),
      key: 'action',
